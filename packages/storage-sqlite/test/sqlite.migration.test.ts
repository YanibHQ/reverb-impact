...
describe('SQLite migrations and runtime mode', () => {
  it('applies forward migrations idempotently and enables WAL', async () => {
    ...
    expect(first.migrationVersions()).toEqual([1, 2, 3, 4, 5]);
    expect(first.pragma('journal_mode')).toMatchObject({ journal_mode: 'wal' });
    first.close();

    const reopened = new SqliteStore(path);
    expect(reopened.migrationVersions()).toEqual([1, 2, 3, 4, 5]);
    reopened.close();
  });
  ...
  it('supports two readers over the same WAL database', async () => {
    ...
    expect(left.migrationVersions()).toEqual([1, 2, 3, 4, 5]);
    expect(right.migrationVersions()).toEqual([1, 2, 3, 4, 5]);
    left.close();
    right.close();
  });
});
